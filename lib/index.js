...
cmbot.prototype.eventNoSong = function() {
	...
	this.bot.on('nosong', function(data) {
		...
		if(typeof cmbot.customEvents['nosong'] == 'function') {
			cmbot.customEvents['nosong'](data);
		}
	});
};
...
cmbot.prototype.eventBootedUser = function() {
	...
	cmbot.bot.on('booted_user', function(data) {
		if(typeof cmbot.customEvents['booted_user'] == 'function') {
			cmbot.customEvents['booted_user'](data);
		}
	});
};
...
cmbot.prototype.eventSnagged = function() {
	...
	cmbot.bot.on('snagged', function(data) {
		if(typeof cmbot.customEvents['snagged'] == 'function') {
			cmbot.customEvents['snagged'](data);
		}
	});
};
	...
	'setdjtimeout': {
		command: function(options) {
			if(options.arg != parseInt(options.arg)) {
				options.cmbot.bot.pm("Invalid syntax. Please provide a number of minutes.", options.userid);
			} else {
				options.cmbot.settings.idleDJTimeout = options.arg == "0" ? false : parseInt(options.arg);
		...
		command: function(options) {
			...
			if(options.cmbot.options.playsMode == 'lastfm') {
				...
				if(mysql !== false) {
					var query;
					if(track === false)
						query = "SELECT s.artist, COUNT(s.id) AS play_count FROM song s JOIN songlog sl ON sl.songid = s.id WHERE LOWER(s.artist) = '" + artist + "' group by s.artist";
					else
						query = "SELECT s.artist, s.track, COUNT(s.id) AS play_count FROM song s JOIN songlog sl ON sl.songid = s.id WHERE LOWER(s.artist) = '" + artist + "' AND LOWER(s.track) = '" + track + "' GROUP BY s.artist";
				...
				if(!options.cmbot.options.sqlite.enabled) {
					...
					var query = "SELECT s.artist, COUNT(s.id) AS play_count FROM song s LEFT JOIN songlog sl ON sl.songid = s.id WHERE LOWER(s.artist) = '" + artist + "' group by s.artist";
					var trackQuery = "SELECT s.artist, s.track, COUNT(s.id) AS play_count FROM song s LEFT JOIN songlog sl ON sl.songid = s.id WHERE LOWER(s.artist) = '" + artist + "' AND LOWER(s.track) = '" + track + "' GROUP BY s.artist";
