...
//		log("users: ", data.users);
		...
		$(data.users).each(function(index, user) {
			if (typeof cmbot.users[user.userid] != 'object') {
				...
				cmbot.users[user.userid] = new User({userid: user.userid, name: user.name, mod: cmbot.mods[user.userid] == 1 || user.acl > 0, laptop: user.laptop});
			}
		});
...
//		log("With " + numUsers + " users, num = " + num);
		if (userid != cmbot.options.bot.userid) {
			if (data.room.metadata.upvotes >= num && !cmbot.session.lamed) 
				cmbot.bot.vote('up');
			if (data.room.metadata.upvotes > cmbot.options.snag_threshold && !cmbot.session.loved && cmbot.currentSong.room.metadata.current_song.metadata.song != 'Untitled' && currentSong.room.metadata.current_song.metadata.artist != 'Unknown') {
				log("Yoinking track");
				cmbot.yoinkTrack();
				cmbot.bot.snag();
				cmbot.session.loved = true;
...
//			log("User laptop: " + newDj.laptop);
			if(!newDj.mod && (newDj.laptop == 'iphone' || newDj.laptop == 'android') && cmbot.settings.mobileWhitelist[newDj.userid] == undefined) {
...
//					log("setting count: " + arg);
	...
	'addme': {
		command: function(options) {
			...
			if (!options.cmbot.isFFA()) {
				if((options.cmbot.users[options.userid].laptop == 'iphone' || options.cmbot.users[options.userid].laptop == 'android') && options.cmbot.settings.mobileWhitelist[options.userid] == undefined) {
