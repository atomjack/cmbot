		...
		command: function(options) {
			...
			if(options.cmbot.options.playsMode == 'lastfm') {
				...
				if(!options.cmbot.options.sqlite.enabled) {
					...
					var trackQuery = "SELECT s.artist, s.track, COUNT(s.id) AS play_count FROM song s LEFT JOIN songlog sl ON sl.songid = s.id WHERE LOWER(s.artist) = '" + artist.toLowerCase() + "' AND LOWER(s.track) = '" + track.toLowerCase() + "' GROUP BY s.artist";
