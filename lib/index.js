			...
			cmbot.lastfm.getTags({
				...
				callback: function(result) {
					if (result.success) {
						...
						if (tags.length > 0) {
							if (cmbot.session.tagLog.length >= cmbot.session.tagLogLimit - 1)
								cmbot.session.tagLog.shift();
							cmbot.session.tagLog.push(tags);
...
cmbot.prototype.getMysqlClient = function() {
	...
	if (cmbot.mysql === false) {
		try {
			cmbot.mysql = require('mysql').createClient({
...
//	log("songs: ", previous_songs);
	if (previous_songs.length > 0 && cmbot.session.tagLog.length < cmbot.session.tagLogLimit) {
...
//					});
		...
		log("Done getting tags: ", cmbot.session.tagLog);
	}
}
...
cmbot.prototype.selectNextSong = function() {
	...
	if (cmbot.session.tagLog.length == 0) {
		...
		cmbot.bot.playlistAll(function(res) {
			...
			cmbot.matchSongTags(songs, function(songsWithTags) {
				...
				var tags = {};
				for (var i = 0; i < cmbot.session.tagLog.length; i++) {
					var song = cmbot.session.tagLog[i];
