...
//		currentSong.room.metadata.djs.push(newDj.userid);
		...
		if(newDj.userid != cmbot.options.bot.userid) {
			...
			if(cmbot.session.djing && (cmbot.session.djs.length == cmbot.session.max_djs) && (!cmbot.session.enforcement || cmbot.session.ffa)) {
...
cmbot.prototype.eventHttpRequest = function() {
	...
	this.bot.on('httpRequest', function (request, response) {
		if(typeof this.customEvents[event] == 'object') {
			for(var module in this.customEvents['httpRequest']) {
				if(typeof this.customEvents['httpRequest'][module] == 'function')
					this.customEvents['httpRequest'][module](this, request, response);
			}
		}
	});
};
