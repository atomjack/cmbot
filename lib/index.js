...
//		log("received speak: ", data);
	// 	Respond to "/urban" command (uses urbandictionary.com)
		if(data.text.match(/^\/urban/i)) {
			var queryResponse = '';
			var sSplit = data.text.split("/urban");
			var searchTerms = sSplit.pop().replace(/\s/g, "%20").replace("%20", "").trim();
			log("SEARCH: " + searchTerms);
			// https://api.urbandictionary.com/v0/define?term=one%20two%20three
			...
			response.on('end', function() {
				...
				if(typeof ret.list[0].definition === "undefined") {
					...
					cmbot.bot.speak("DEFINITION: Come on, you're failing at a simple task here.");
				} else {
					log("DEFINITION: " + ret.list[0].definition);
					cmbot.bot.speak("DEFINITION: " + ret.list[0].definition);
				}
				} catch(err) {
			...
			cmbot.bot.speak("Got error: " + e.message);
			});
		}
