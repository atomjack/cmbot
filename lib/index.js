	...
	this.bot.on('add_dj', function(data) {
		...
		if(cmbot.settings.deckshitlist[newDj.userid] != undefined) {
			...
			cmbot.bot.pm("You are banned from DJ'ing" + (cmbot.settings.deckshitlist[newDj.userid].reason == "" ? "." : ": " + cmbot.settings.deckshitlist[newDj.userid].reason), newDj.userid, function(result) {
				if(!result.success) {
					cmbot.bot.speak("You are banned from DJ'ing, " + options.cmbot.users[options.userid].name + (cmbot.settings.deckshitlist[newDj.userid].reason == "" ? "." : ": " + cmbot.settings.deckshitlist[newDj.userid].reason));
				}
			});
		}
...
//					log("setting count: " + arg);
	...
	'deckshitlist': {
			...
			if(options.arg == "")
				...
				if(user === false)
					...
					options.cmbot.settings.deckshitlist[user.userid] = {
						...
						reason: reason,
						originator: {
							name: options.cmbot.users[options.userid],
		...
		modonly: true,
		pmonly: true,
		help: "Ban a user from DJ'ing. I will automatically escort them from the decks if they step up."
	},
	'deckunshitlist': {
		command: function(options) {
			var text;
