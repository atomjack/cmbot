...
//		usersOnDecks = data.room.metadata.djs;
		...
		var banned = false;
		$.each(cmbot.settings.bannedArtists, function(key, val) {
			var checkArtist = key.toLowerCase();
		...
		if(cmbot.options.scrobble === true) {
...
cmbot.prototype.autodj = function() {
	...
	if(cmbot.session.djing)
		...
		log("AutoDJ is off.");
	else if(cmbot.session.max_djs - cmbot.session.djs.length < 2)
		log("There are not 2 or more spots free. (max djs: " + cmbot.session.max_djs + ", num djs: " + cmbot.session.djs.length + ")");
...
cmbot.prototype.doTrigger = function(userid, command) {
	...
	cmbot.bot.speak(cmbot.settings.triggers[command].replace('%me%', '@' + cmbot.users[userid].name).replace('%dj%', '@' + djName));
};
		...
		path.exists(file, function(exists) {
			if(!exists) {
				fs.writeFile(file, "{}", function(err) {
					log("Settings or DJ file is not writable");
					process.exit(1);
				});
...
//						bot.speak("Trigger saved.");
	...
	'shitlist': {
		command: function(options) {
			...
			} else if(user.userid == this.options.master_userid) {
				text = 'I\'m sorry, Dave. I afraid I can\'t do that.'; 
			} else {
				if(user.mod) {
	...
	'settimezone': {
		command: function(options) {
			...
			if(cmbot.timezones[options.arg] == undefined) {
				options.cmbot.bot.pm("Invalid syntax.", options.userid);
			} else {
...
//	},
	...
	'unwarn': {
		command: function(options) {
			if(options.cmbot.session.warned) {
				var warnUser = options.cmbot.users[currentSong.room.metadata.current_song.djid];
		...
		command: function(options) {
			...
			if(options.arg == '')
				...
				if(!options.cmbot.session.autodj)
					...
					clearTimeout(options.cmbot.timers.autodj);
					options.cmbot.timers.autodj = false;
